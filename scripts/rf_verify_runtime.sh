...
if (( is_zip || is_zst )); then
  # Respect Termux/Android temp layout
  TMPDIR="${TMP:-$HOME/tmp}"
  mkdir -p "$TMPDIR"
  ...
  if (( is_zip )); then
    ...
    # Large-window zstd support for rf-runtime tarballs
...
for name in "${x64_need[@]}"; do
  hit="$(
    (find "$R/x86_64-linux" -maxdepth 3 -type f -name "$name" 2>/dev/null || true) | head -n1
  )"
  if [[ -n "$hit" ]]; then
    echo "ok $hit"
    ((have_x64++)) || true
  else
    echo "MISS $R/x86_64-linux/**/$name"
  fi
done
...
for name in "${x86_need[@]}"; do
  hit="$(
    (find "$R/i386-linux" -maxdepth 3 -type f -name "$name" 2>/dev/null || true) | head -n1
  )"
  if [[ -n "$hit" ]]; then
    echo "ok $hit"
    ((have_x86++)) || true
  else
    echo "MISS $R/i386-linux/**/$name"
  fi
done
...
for d in "$R/wine64" "$R/wine32"; do
  if [[ -d "$d" ]]; then
    echo "ok $d"
    find "$d" -maxdepth 1 -type f | sed 's/^/ - /' | head
  else
    echo "MISS $d"
  fi
done
